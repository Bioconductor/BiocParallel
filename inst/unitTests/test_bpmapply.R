...
test_bpmapply_Params <- function() {
    ...
    for (catch.errors in c(FALSE, TRUE)) {
      ...
      for (SIMPLIFY in c(FALSE, TRUE)) {
        for (USE.NAMES in c(FALSE, TRUE)) {
          ...
          .fork_not_windows(expected, bpmapply(identity, x, USE.NAMES=USE.NAMES, SIMPLIFY=SIMPLIFY, BPPARAM=param))
        }
      }
    }
  }
  ...
  for (param in params) {
    .fork_not_windows(expected, bpmapply(f, x, MoreArgs = list(m = 1), BPPARAM=param))
  }
