...
date: "Edited: March 22, 2018; Compiled: `r format(Sys.time(), '%B %d, %Y')`"
vignette: >
...
The class is an interface to `batchtools` the package from within `BiocParallel`. This aims to
replace BatchjobsParam as `BiocParallel`'s class for computing on a high performance
...
The interface allows you to replace the BatchJobsParam interface, and
allow more intuitive usage of your high performance cluster with `BiocParallel`.
...
-   `registryargs` The 'registryargs' argument takes a list of arguments to create a
    ...
    + `file.dir` Path where all files of the registry are
