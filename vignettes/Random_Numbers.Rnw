...
Attach \BiocParallel{} and ensure that the version is greater than 1.27.6
...
For reproducible calculation, use the \Rcode{RNGseed=} argument to each
...
\subsection{Use with \Rcode{bpiterate()} and \Rcode{bptry()}}
...
This function is used to create a function that, on each invocation,
...
\Rcode{bplapply()} or \Rcode{bpiterate()} allows for the possibility for
graceful recovery from errors. Here a buggy \Rcode{FUN1()} produces an
error for the second element. This is shown in the result.
...
When \Rcode{RNGseed=} is used, the global random number stream is ignored.
...
When \Rcode{RNGseed=} is not used, the global random number stream is
used, and the results for a given seed are the same as if
\Rcode{RNGseed=} had been used.
...
Regardless of whether \Rcode{RNGseed=} is used (or whether \Rcode{FUN}
uses a random number stream), the global random number stream behaves
as though it had been called once.

<<set-seed-increment>>=
set.seed(100)
second_element <- runif(2)[2]

set.seed(100)
result14 <- bplapply(1:3, runif, BPPARAM = SerialParam(RNGseed = 200))
stopifnot(
    identical(second_element, runif(1))
)

set.seed(100)
result15 <- bplapply(1:3, runif, BPPARAM = SerialParam())
stopifnot(
    identical(second_element, runif(1))
)
@ 
...
state. Internally, \Rcode{bplapply()} invokes \Rcode{bpstart()} to
establish the computational environment. \Rcode{bpstart()} can be
called explicitly, e.g., to allow workers to be used across calls to
\Rcode{bplapply()}.

The cluster random number stream is initiated with \Rcode{bpstart()}. Thus
...
bpstop(param)
stopifnot(
    !identical(result16, result17)
)
@ 

The interplay between \Rcode{bpstart()} and \Rcode{set.seed()} needs to
be considered carefully when \Rcode{RNGseed=} is not used, and for this
reason the recommended practice is to always use \Rcode{RNGseed=}.

<<set.seed-and-bpstart>>=
set.seed(200)
param <- SerialParam()  # RNGseed = NULL by default; global stream ignored

set.seed(100)
param <- bpstart(param) # use global stream to seed parallel streams
result18 <- bplapply(1:3, runif, BPPARAM = param)
result19 <- bplapply(1:3, runif, BPPARAM = param)
bpstop(param)

stopifnot(
    identical(result1, result18),
    !identical(result1, result19)
)
@

\subsection{Relationship between \Rcode{bplapply()} and \Rcode{lapply()}}

The results from \Rcode{lapply()} are different from the results from
\Rcode{bplapply()}, even with the same random number seed. This is
...
additional details. This random number generator is useful because it
can be used to generate independent streams and substreams of random
numbers. In \BiocParallel{}, each call to \Rcode{bpstart()} creates a
new stream from the L'Ecuyer-CMRG generator. Each element in
\Rcode{bplapply()} or \Rcode{bpiterate()} creates a new substream. Each
application of \Rcode{FUN} is therefore using the L'Ecuyer-CMRG random
number generator, with a substream that is independent of the
substreams of all other elements.

Within the user-supplied \Rcode{FUN} of \Rcode{bplapply()} or
\Rcode{bpiterate()}, it is a mistake for the user to use
\Rcode{RNGkind()} to set a different random number generator, or to
use \Rcode{set.seed()}. This would in principle compromise the
independence of the streams across elements.
