...
\begin{itemize}
  ...
  \item{registryargs} 
    ...
    \begin{description}
      ...
      \item{work.dir} Working directory for R process for running
        jobs.

      \item{packages} Packages that will be loaded on each node.

      \item{namespaces} Namespaces that will be loaded on each node.

      \item{source} Files that are sourced before executing a job.
...
@
  ...
  \item{resources} A named list of key-value pairs to be subsituted into
    the template file; see \Rcode{?batchtools::submitJobs}.
  ...
  \item{template} The template argument is unique to the
    ...
    a default template is selected from the batchtools package. We
    recommend that the user define a template which works on their
    cluster, and supply a path to the template argument.

  \item{log} The log option is logical, TRUE/FALSE. If it is set to
    TRUE, then the logs which are in the registry are copied to
    directory given by the user using the \Rcode{logdir} argument.

  \item{logdir} Path to the logs. It is given only if
    \Rcode{log=TRUE}.

  \item{resultdir} Path to the directory is given when the job has
    files to be saved in a directory.
