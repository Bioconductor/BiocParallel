...
provides only serial evaluation on Windows.
...
Developers wishing to invoke back-ends other than \Rcode{MulticoreParam},
or to write code that works across Windows, macOS and Linux, no longer need to take
special care to ensure that required packages, data, and functions are available
and loaded on the remote nodes. By default, \Rcode{SnowParam} will export global
variables to the workers due to the default \Rcode{exportglobals=TRUE}.
...
If the package is installed on a server used by multiple users, then the default
value of cores used can sometimes lead to many more tasks being run than the server
has cores if two or more users run a parallel-enabled function simultaneously. A
more conservative number of cores than all of them minus 2 may be desirable, so that
one user does not take all of the cores unless they explicitly specify so. This can be
implemented with environment variables. Setting \Rcode{R_PARALLELLY_AVAILABLECORES_FALLBACK}
or \Rcode{BIOCPARALLEL_WORKER_NUMBER} for all system users to the number of cores
divided by the typical number of concurrent users is a reasonable approach to
