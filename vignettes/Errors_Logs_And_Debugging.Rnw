...
each element of \Rcode{X} to be executed separately (6 tasks) vs chunked.
...
X <- list(1, 2, "3", 4, 5, 6)
param <- SnowParam(3, stop.on.error = FALSE)
@
...
\begin{verbatim}
## bplapply(X.redo, sqrt, BPREDO = res, param=param)
## 
...
\begin{verbatim}
param <- SnowParam(stop.on.error=FALSE)
res <- bplapply(list(c(1,3), 5, "6"), fun1, BPPARAM = param)
res
