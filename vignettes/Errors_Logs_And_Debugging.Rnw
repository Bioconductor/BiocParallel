...
\Rcode{X} is divided into two tasks and evaluated. Task 1 fails after
the second element, stopping further computation in that task. All
successfully completed tasks are returned and can be accessed by
\Rcode{bpresult()}.
...
Using \Rcode{stop.on.error=FALSE}, all tasks are fully evaluated.
...
\Rcode{bptry()} is a convenient way to evaluate a \Rcode{bpapply}-like
expression, returning the evaluated results without signalling an
error.
