...
tasks failed. In this example we use \Rcode{tryCatch()} to report the
error with failing, to the second element fails.
...
The partially evaluate results are available by retrieving the
\Rcode{result} attribute
...
<<logs_FUN>>=
FUN <- function(i) {
  flog.debug(paste("value of 'i':", i))

  if (!length(i)) {
      flog.warn("'i' has length 0")
      NA 
