...
\begin{description}
  ...
  \item{support of \Rcode{foreach}:}
    The \Rcode{foreach} and \CRANpkg{iterators} packages are fully supported 
...
The \BiocParallel functions use the registered back-ends for evaluation.
The default is the entry the top of the list.
<<quickstart_registry>>=
registered()
@

When a \BiocParallel{} function such as \Rcode{bplapply} is invoked with
no \Rcode{BPPARAM} argument the default back-end is used.
<<quickstart_bplapply, eval=FALSE>>=
bplapply(1:4, FUN)
@

Re-run the job using a 2-worker PSOCK cluster instead of the default.
<<quickstart_bplapply_param, eval=FALSE>>=
...
Add a specialized instance with \Rcode{register}.
<<register_BatchJobsParam>>=
register(BatchJobsParam(workers = 10))
@

The act of registering moves the entry to the top of the list.
BatchJobsParam is now the default.
...
\subsubsection{Query and control parallel evaluation environment}
...
The following functions assist in recovering from errors.
...
recommended back-end is multi-core which uses forked processes. Because
all workers share the same memory, it is not necessary to pass large 
objects to or load common packages on all workers.

In this example we compute coverage on each element of a GRangesList.

Create a GRangesList with regions of interest (in practice this could be 
large).
<<forking_grl, message=FALSE>>=
library(GenomicRanges)
gr <- GRanges("chr14", IRanges((1000:4999)*5000, width=1000))
grl <- splitAsList(gr, rep(1:4, each=1000))
@

The function computes coverage on a single element of the GRangesList.
<<forking_FUN>>=
FUN <- function(i) {
    coverage(grl[[i]])
}
...
<<forking_bplapply, eval=FALSE>>=
bplapply(seq_along(grl), FUN, BPPARAM=MulticoreParam())
@

Note that the GRangesList object was not explictly passed in the call
to \Rcode{bplapply} and FUN did not have to load the \Biocpkg{GenomicRanges} 
package for access to the \Rcode{coverage} function.

\subsubsection{Clusters: SnowParam, DoparParam}
On a Windows machine forking is not an option and processes must be 
spawned via a cluster. Back-end choices are a Snow cluster configured with 
\Rclass{SnowParam} or the \Rclass{DoparParam} for use with the 
\Rpackage{foreach} package.

To run the same coverage example on a cluster we need to modify FUN to 
take the GRangesList as an argument and load \Biocpkg{GenomicRanges}.
<<cluster_FUN>>=
FUN <- function(i, grl) {
    library(GenomicRanges)
    coverage(grl[[i]])
...
<<cluster_snow_param, eval=FALSE>>=
p <- SnowParam(workers = 2)
@

Call \Rcode{bplapply} with the \Rcode{SnowParam} and pass the GRangesList
as an argument. 
<<cluster_bplapply, eval=FALSE>>=
bplapply(seq_along(grl), FUN, BPPARAM = p, grl = grl)
@

The FUN written for the cluster adds some overhead due to the passing of the 
GRangesList and time due to the loading of \Biocpkg{GenomicRanges} on 
each worker. This approach, however, has the advantage that it works on most
platforms and does not require a coding change when switching between windows
and non-windows machines. 
