...
communicate with each other and to which the user has log-in access. This 
...
Star Cluster toolkit must be installed and a config file must
...
RStudio or SSH. If you connect via RStudio an \R{} process starts 
automatically.

A primary difference between using the AMI cluster vs a cluster with formal 
scheduler such as SLURM is how jobs are submitted. When using the AMI, jobs 
are started from an interactive \R{} session when logged into the master node.
As demonstrated in the section on `Clusters with Schedulers`, when using
formal schedulers jobs are submitted in batch mode from the command line.

The AMI cluster workers can communicate via SSH, SunGrid Engine or MPI.
For MPI use a \Robject{SnowParam} with \Rcode{type = MPI}. Both SSH and 
SunGrid Engine require a \Robject{BatchJobsParam} but differ slightly in 
the set-up.

For SSH, the individual nodes are defined as cluster functions. The
`workers` should be the number of nodes and `ncpus` the number
...
\R{} and \Bioconductor{}} 
(\url{http://www.imstat.org/sts/future_papers.html}) by Michael Lawrence 
