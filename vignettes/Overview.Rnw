...
Create a test function that returns the hostname and square root of ``x''.
%% 
<<quick_start_FUN>>=
FUN <- function(x) {
    hname <- system("hostname", intern=TRUE)
    paste0("host: ", hname, ", result = ", round(sqrt(x), 4))
}
@
%% 

The \BiocParallel functions use the registered back-ends for evaluation.
The default back-end is at the top of the registry list.
%% 
<<quick_start_registry>>=
registered()
@
%%

Invoke a \BiocParallel{} function such as \Rcode{bplapply}.
<<quick_start_bplapply>>=
bplapply(1:4, FUN)
@

Optionally define a back-end for a particular configuration 
with a BiocParallelParam.
<<quick_start_param>>=
...
Once the Snow cluster is registered it moves to the top of the list.
<<quick_start_registry2>>=
register(param)
registered()
@

Calling \Rcode{bplapply} now uses a  2-worker PSOCK cluster.
<<quick_start_bplapply2>>=
bplapply(1:4, FUN)
@
...
\subsection{\Rcode{BiocParallelParam} instances}
\Rclass{*Param} objects describe parallel evaluation environments.
...
\subsubsection{Parallel versions of *apply}

%% what do these call, mclapply(), parLapply(), pvec()
...
\subsubsection{Query and control evaluation enviornments}
    \begin{description}
...
%% - shared memory vs shared enviornments
%% - mclapply and fork clusters not on windowss (shared enviornments)
%% cluster
%% - SOCK or fork

%%Note that whereas with mclapply all the packages and objects we use are
%%automatically available
%%on the workers, this is not in general7
%%the case with the parLapply approach. There is often
%%a delicate choice of where to do the computations: for example we could compute
%%cd4.mle on
%%the workers (as above) or on the master and send the value to the workers. We
%%illustrate the
%%latter by the following code

%% example of code for cluster vs forking
...
\subsection{Limiting resource comsumption}
