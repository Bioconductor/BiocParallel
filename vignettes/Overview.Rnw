...
Environment specific back-ends can be defined for any of the
registry entries. Here we create a 2-worker PSOCK cluster and
use it in place of the default.
<<quickstart_snow>>=
...
In this example we count overlaps between BAM files and a defined
set of ranges. Create a GRanges with regions of interest (in practice this 
could be large).
...
To re-run the counting example we need to modify FUN to run in a cluster 
environment (not shared memory). `gr` data is passed as an argument 
and \Biocpkg{GenomicAlignments} is loaded on each worker.
<<cluster_FUN>>=
...
Define a 2-worker PSOCK Snow cluster.
<<cluster_snow_param>>=
...
GRanges and time due to loading \Biocpkg{GenomicAlignments} on each 
...
The term \emph{ad hoc} cluster defines a group of machines that can communicate
with each other and to which the user has log-in access. This example uses a 
group of compute machines ("the rhinos") on the FHCRC network. 
...
allocated resources. For SLURM, we might request space for 4 tasks 
...
An AMI optimized for running \Bioconductor{} in the Elastic Compute
Cloud (EC2) is available at
...
General steps involved in setting up an instance of the AMI.
\begin{enumerate}
  \item Create an account with Amazon Web Services
  \item Create a key pair
    If you plan to connect to the AMI via SSH you need to create a
    key pair file. If you plan to connect via a browser with
    RStudio you can skip this step.
  \item Start the AMI
    The documentation has separate links for starting the AMI via SSH 
    or RStudio. This step is where you choose the size / type of
    computing resource. 
...
suited to \R{} programs. A manuscript titled `Scalable Genomics with
\R{} and \Bioconductor{}, by Michael Lawrence and Martin 
Morgan, \url{http://www.imstat.org/sts/future_papers.html} 
reviews various approaches and their implementation in \Bioconductor{}
packages. Areas covered include scalable processing, summarization and 
visualization. Presented techniques are well-established and include 
restricting queries, compressing data, iterating, and parallel computing. 
