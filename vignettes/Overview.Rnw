...
\date{Edited: 16 February, 2014; Compiled: \today}
...
parallel evaluation within \Bioconductor. Enable its use by loading
the library
%% 
<<BiocParallel>>=
...
\end{enumerate}
%% 
Loading the \BiocParallel{} package actually perofrms steps 1 and 2,
...
\begin{description}
\item[\Rcode{SerialParam}] Evaluate \BiocParallel-enabled code without
  parallelization. This is very useful when writing new scripts and
...
algorithms may specify a prefered back-end, and different back-ends
