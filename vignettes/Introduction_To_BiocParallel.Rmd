...
-  `SerialParam`:
    ...
    Evaluate [*BiocParallel*][]-enabled code with parallel evaluation disabled. This
...
-   `SnowParam`:
    ...
    Evaluate [*BiocParallel*][]-enabled code across several distinct instances, on one or
...
-   `BatchtoolsParam`:
    ...
    Evaluate [*BiocParallel*][]-enabled code by submitting to a cluster scheduler like
    SGE.
...
-   `DoparParam`:
    ...
    Register a parallel back-end supported by the [*foreach*](https://cran.r-project.org/package=foreach) package for use with [*BiocParallel*][].
...
spawn processes. [*BiocParallel*][] back-end choices for clusters on a single machine are *SnowParam* for configuring a Snow cluster or the *DoparParam* for use with the *foreach* package.
