...
with release 2.14.0 with inclusion of the parallel package which contains modified versions of
*multicore* and *snow* .
...
*BiocParallel* offers the following conveniences over the 'roll your own' approach to
...
-   cluster scheduling: When the parallel environment is managed by a
    cluster scheduler through *batchtools*, job management and result retrieval are
...
Functions in `BiocParallel` use the registered back-ends for parallel evaluation. The
...
*bashtools* to submit jobs from within *R* the session. One way of doing this is to
