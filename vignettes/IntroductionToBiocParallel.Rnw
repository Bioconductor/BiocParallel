...
\date{Edited: March 1, 2014; Compiled: \today}
...
\begin{itemize}
  ...
  troublesome way to parallelize code. Windows does not 
  ...
  Based on facilities originally implemented in the \CRANpkg{multicore}
  package and subsequently the \CRANpkg{parallel} package in base \R{}.
...
\subsubsection{Error recovery and logging}

Logging and advanced error recovery for \Rcode{SnowParam} and
\Rcode{MulticoreParam} is available in \Rcode{BiocParallel} 1.1.25 and later.
The \Rcode{futile.logger} package is used for logging and supports the setting
of a log threshold and user-specific log messages. Errors and warning messages
are returned from the worker.

\Rcode{BatchJobsParam} uses the default \Rcode{BatchJobs} behavior and
writes error messages to the result files.

The following assist in error handling and logging.

\begin{description}
  \item{\Rcode{bpcatchErrors}: }
    Determine if apply-like functions quite with an error as soon as
    one application fails.

  \item{\Rcode{bpstopOnError}: }
    Determine if all jobs stop as soon as one fails or if all should
    be attempted.
...
\begin{verbatim}
> library(BiocParallel}
> param <- SnoParam(workers=3, type = "MPI") 
\end{verbatim}
