...
MulticoreParam <-
    function(workers=detectCores(), catch.errors=TRUE, setSeed=TRUE,
        ...
        verbose=FALSE, ...)
{
    .MulticoreParam(workers=workers, catch.errors=catch.errors, setSeed=setSeed,
        recursive=recursive, cleanup=cleanup, cleanupSignal=cleanupSignal,
        verbose=verbose, ...)
}
...
setMethod(bpschedule, "MulticoreParam", function(x, ...) {
