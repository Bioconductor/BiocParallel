...
.filterLibraries <-
    ...
    i <- grepl(paste0("(",paste0(loadFunc, collapse = "|"),")"), codes)
    xx <- lapply(
        codes[i],
        function(code){
    ...
    function(fun, ERROR_FUN = capture.output)
{
    ...
    loadFunc <- c("require", "library")
    loadFunc <- loadFunc[loadFunc %in% unknown]
    if (length(loadFunc))
        unknown <- .filterLibraries(codes, unknown, loadFunc, ERROR_FUN)
